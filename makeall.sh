...
browserify --list src/js/lametest.js  | grep lamejs | grep -v node_modules | xargs cat | grep -v -e 'common\..*;' -e 'require(' -e 'module.exports = .*;$' | sed 's/^module.exports = {/var module_exports = {/';
echo 'L3Side.SFBMAX = (Encoder.SBMAX_s * 3);'
echo '//testFullLength();'
echo 'this.Mp3Encoder = Mp3Encoder;'
echo 'this.WavHeader = WavHeader;'
echo '}'
echo "//fs=require('fs');"
echo '//lamejs();'
