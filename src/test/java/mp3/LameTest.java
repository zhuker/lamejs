...
public class LameTest {
    ...
    @Test
    public void testchmode2string() throws IOException {
        String s = LameEncoder.chmode2string(2);
        MPEGMode mpegMode = Enum.valueOf(MPEGMode.class, s);
        Assert.assertEquals(MPEGMode.DUAL_CHANNEL, mpegMode);
    }
...
//        File file = new File("/Users/zhukov/git/tle1.3x/test-data/wav/440880.wav");
        File file = new File("testdata/c00.wav");
        MappedByteBuffer map = mmapReadonly(file);
        map.order(ByteOrder.LITTLE_ENDIAN);
        int dataLen = map.getInt(0x28);
        map.position(0x2c);
        map.limit(0x2c + dataLen);
...
//        samples.limit(1152 * 8);
        Assert.assertEquals(0x0089, samples.get(0));
        Assert.assertEquals(0x00D0, samples.get(1));
        int remaining = samples.remaining();
        ...
        OutputStream out = new BufferedOutputStream(new FileOutputStream("generated_java.mp3"));
        out.write(mp3buf, 0, mp3bufPos);
        out.close();

        MappedByteBuffer mappedByteBuffer = mmapReadonly(new File("testdata/c00.mp3"));
        byte[] expected = new byte[mappedByteBuffer.capacity()];
        mappedByteBuffer.get(expected);
        byte[] actual = Arrays.copyOf(mp3buf, mp3bufPos);
        Assert.assertArrayEquals(expected, actual);
    }

    private MappedByteBuffer mmapReadonly(File file) throws IOException {
        return new FileInputStream(file).getChannel().map(FileChannel.MapMode.READ_ONLY, 0, file.length());
    }
}
